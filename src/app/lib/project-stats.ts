...
export const getOrCreateViewerKey = (): string => {
  if (typeof window === "undefined") {
    return "";
  }
  const storageKey = "visuales:viewer_key";
...
export const recordProjectView = async (
  projectId: string,
  options: RecordProjectViewOptions = {}
): Promise<void> => {
  if (!supabase || !projectId) {
    return;
  }
  const viewerKey = options.viewerKey ?? "";
  const cooldownSeconds = Math.max(5, Math.floor(options.cooldownSeconds ?? 45));
  const { error } = await supabase.rpc("record_project_view_guarded", {
    p_project_id: projectId,
    p_viewer_key: viewerKey,
    p_min_seconds: cooldownSeconds,
  });

  if (error && /record_project_view_guarded/i.test(error.message)) {
    // Backward-compat fallback until the migration is applied in Supabase.
    const fallback = await supabase.rpc("record_project_view", { p_project_id: projectId });
    if (fallback.error) {
      console.error("record_project_view fallback failed", { projectId, error: fallback.error.message });
    }
    return;
  }

  if (error) {
    console.error("record_project_view failed", { projectId, error: error.message });
  }
};
